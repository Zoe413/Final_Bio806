...
WeatherData2023<- read.csv("C:/Users/zrrob/OneDrive/Desktop/Kiwiberry/Historical Temp Data/WeatherData2023.csv")
...
ggplot(data = WeatherData2023, aes(x=Time)) +
  ...
  ggtitle("Figure 1. Temperature vs Accumulated Chill Portions Over Time")
...
chill<- read.csv("C:/Users/zrrob/OneDrive/Desktop/Kiwiberry/Chill requirement methods/chill.csv", header = TRUE)
...
barchartCP <- ggplot(data = ChillData, aes(x = Treatment, y = Percent_BudsBurst)) +
  geom_col() +
  labs(x = "Number of Chill Portions", y = "Percent Budburst") +
  ggtitle("Figure 2. Budburst 4 Weeks Post-Collection")

ggsave("barchartCP.png", plot = barchartCP, width = 20, height = 16)
```
...
As shown in the ANOVA results below, there was no significant difference in the date of spring budbreak between different varieties of kiwiberry, or between laterals that faced different directions. There was also no significant difference in the date of spring budbreak between different gibberellic acid treatments.

```{r, echo=FALSE, results=FALSE}
data<- read.csv("C:/Users/zrrob/OneDrive/Desktop/Kiwiberry/GA Methods/gadata_splitsplit_2024_v2.csv")
...
```         
                         Sum Sq Df Mean Square F value  Pr(>F)
```

```         
block                     68.59  3  22.8633    2.494 0.2363104
```
...
```         
ga_trtrmt                 55.37  3  18.457     1.4061 0.25733
```

```         
variety:ga_trtrmt         40.97  3  13.66      1.0406 0.38668
```

```         
aspect:ga_trtrmt           9.74  3  3.25       0.2474 0.86261
```

```         
variety:aspect:ga_trtrmt  35.97  3  11.99      0.9135 0.44441
```
...
ggplot(GA2024, mapping = aes(x=ga_trtrmt, y=budbreak))+
  ...
  ggtitle("Figure 3. Effect of Winter Gibberellic Acid Application on Spring Budbreak")
# Variety versus budbreak date
ggplot(GA2024, mapping = aes(x=variety, y=budbreak))+
  geom_boxplot()+
  xlab("Kiwiberry Variety")+
  ylab("Day of 30% Budbreak, Day 0=4/18/24")+
  ggtitle("Figure 4. Effect of Kiwiberry Variety on Spring Budbreak")
```
